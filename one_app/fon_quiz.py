...
class FonquizScreen(BaseScreen):
    def on_enter(self):
        ...
        for litera in wszystkie_litery_alf:
            ...
            layout.score = NumericProperty(0)

            for Przycisk, wybrana_litera in zip(buttons, wybrane_litery):
                Przycisk.text = wybrana_litera
                if litera == wybrana_litera:
                    Przycisk.bind(on_release=action(self, Przycisk, odp_true, buttons, layout.ids.wynik, True))
                else:
                    Przycisk.bind(on_release=action(self, Przycisk, odp_false, buttons, layout.ids.wynik, False))
